...
def square_selection(the_board,turn):
    square = pyinputplus.inputChoice(
        ["top-L", "top-M", "top-R", "mid-L", "mid-M", "mid-R", "down-L", "down-M", "down-R"],
...
def get_single_player_game(the_board):
    name = input("Wprowadź swoje imię: ")
    pattern_printing()
    turn = "X"
    if turn == "X":
        for i in range(9):
            square = square_selection(the_board,turn)
...
def get_two_players_game(the_board):
